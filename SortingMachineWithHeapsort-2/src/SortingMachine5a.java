...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        // create a variable to temporarily store an entry
        if (i != j) {
            T temp = array[i];
            // swap the entries
            array[i] = array[j];
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        // initialize the left and right children of the root
        int leftChild = 2 * top + 1;
        int rightChild = leftChild + 1;

        // create a variable to hold the smaller child
        int smaller;

        // identify the smaller of the two children and compare it to the root
        if (leftChild <= last) {
            smaller = leftChild;
            if (rightChild <= last) {
                if (order.compare(array[rightChild], array[smaller]) < 0) {
                    smaller = rightChild;
                }
            }

            /*
             * if it turns out that the smallest child is less than the root,
             * exchange entries and make the smallest child the root.
             */
            if (order.compare(array[top], array[smaller]) > 0) {
                exchangeEntries(array, top, smaller);

                /*
                 * call siftdown to recurse through the subtree. use the
                 * smallest child as the root.
                 */
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * check if there are multiple children and recursively navigate through
         * the subtrees.
         */
        // right child loop
        ...
        // call siftDown to fulfill the heap condition.
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // iterate through the queue and place its elements into the array
    ...
    public final void changeToExtractionMode() {
        ...
        /*
         * the sorting and building process starts in extraction mode. Ensure
         * that the heapsize variable has the same size as the heap that is
         * being built.
         */
    ...
    public final T removeFirst() {
        ...
        // extract the last child in the heap
        T extracted = this.heap[0];
        // make the last child the root
        exchangeEntries(this.heap, 0, this.heapSize - 1);
        this.heapSize--;

        // sift down the root if it is too large
    ...
    public final int size() {
        ...
        /*
         * if in insertionMode, the SortingMachine is manipulating a queue so
         * use length method. Else, if not in insertionMode, the SortingMachine
         * is manipulating an array so use heapSize.
         */
        if (this.insertionMode) {
